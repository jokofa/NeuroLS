...
public:
    ...
    // Updates the cummmulative distance and demands for each node and returns the resulting feature vector
    void update_dynamic_node_data();
    std::vector<std::vector<double>> get_dynamic_node_data();
    ...
    void use_initial_solution(std::vector<std::vector<int>> sol);

    // Solution display
    void show_routes();
    void show_route(int k);
    std::vector<std::vector<int>> get_routes();
    std::vector<int> get_route(int k);
    void summary();

    double call_clarke(double lambda, bool use_neighbor_list);
    ...
    double Single_solve(int local_operators, int rule, double err_max,
        int iters, bool converge);

    double Single_node_solve(int node, int local_operators, int rule, 
        double err_max, int iters, bool converge);

    double detailed_solve(int local_operators, int rule, std::vector<int>node,
        int steps, int iters, double err_max, bool converge);
    ...
    //custom accept solution solution
    //bool accept_solve(bool accept);
    bool reject_move();
    ...
    double get_previous_route_length();
    ...
    void add_time_constraints(std::vector<std::vector<double>> time_windows, std::vector<double> service_times);
    ...
    void set_max_num_routes(int num);
...
private:  
    ...
    int random_seed;
    int max_number_routes;

    //hold old solution
    int* previous_sol_buff;
    double previous_total_route_length;
    ...
    int targeted_osman_perturb(int num, int *nodelist, double alpha);
    ...
    inline bool check_savings(VRPMove *M, int rules){
        ...
        if(M->savings>-VRPH_EPSILON && M->savings < VRPH_EPSILON)
            return false;
#endif
        //if its better instantly accept it regardless
