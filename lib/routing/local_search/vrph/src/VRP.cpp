...
VRP::VRP(int n)
{
    ...
    /// where n includes the depot node
    ///

    n--;    // num_nodes should be number of customer nodes without depot
    ...
    previous_sol_buff = new int[n + 2];
    random_seed= 1234;
    
    max_number_routes = 10000;
    ...
    previous_total_route_length = 0.0;
...
VRP::~VRP()
{
    ...
    // no longer private, we don't need to dealloc
    // delete [] this->nodes;
...
void VRP::set_random_seed(int seed = 9){
    this -> random_seed = seed;
    lcgseed(seed);
}


void VRP::update_dynamic_node_data(){

    ///
    /// Updates the cummulative distance and demand for all nodes
    ///
    ...
    //printf("-----------------------------------------------\n");
    //printf("Updating Dynamic Node Data ...\n");
    i = 1;
    cnt = 0;
    ...
    //First step we update current node based on leaving the depot
    running_load = nodes[current_node].demand;

    //add distance depot to next node
    running_distance += d[VRPH_DEPOT][current_node];

    // Set values
    nodes[current_node].cummulative_distance = running_distance;
    nodes[current_node].cummulative_demand = running_load;

    nodes[current_node].cummulative_distance_back = this->get_total_route_length()  -  running_distance;
    nodes[current_node].cummulative_demand_back = route[current_route].load - running_load + nodes[current_node].demand;

    cnt++;

    // loop to update the next nodes based on the current nodes
    while(route_start != 0 && i<num_nodes+1)
    {
        ...
        if(next_array[current_node]>0)
        {
            ...
            // update the distance and demand
            running_distance += d[current_node][next_node_number];
            running_load += nodes[next_node_number].demand;

            // Set distance to next node
            nodes[next_node_number].cummulative_distance = running_distance;
            nodes[next_node_number].cummulative_demand = running_load;

            nodes[next_node_number].cummulative_distance_back = this->get_total_route_length()  -  running_distance;
            nodes[next_node_number].cummulative_demand_back = route[current_route].load - running_load + nodes[next_node_number].demand ;
        ...
        else
        {
            ...
            running_distance += d[current_node][VRPH_DEPOT];
            //add distance depot to next node
            running_distance += d[VRPH_DEPOT][next_node_number];
...
int VRP::get_dummy_index() {
    return this->dummy_index;
}
...
int* VRP::get_perm() {
    ...
    // We need this before starting heuristic solutions
    int* perm;
    perm = new int[this->num_nodes];
    int j = VRPH_ABS(this->next_array[VRPH_DEPOT]);
...
double VRP::call_clarke(double lambda, bool use_neighbor_list){

    ClarkeWright CW(this->get_num_nodes());
    CW.Construct(this, lambda, use_neighbor_list);
...
{    
    ...
    {   
        ...
        for(j=0;j<=n+1;j++){
            ...
            std::cout << j << ", ";
            if (i == 22) {
                std::cout << "service time:" << nodes[j].service_time << ";  ";
                std::cout << "\ni=" << i << "  ";
                std::cout << "\nj=" << j << "  ";
                double dd;
                dd = VRPDistance(
                ...
                std::cout << "dd:" << dd << " terminateeeee";
                d[i][j] = 1.1111;
                std::cout << "dd:" << d[i][j] << " terminateeeee222";
            }
            */

            this->d[i][j]=VRPDistance(
                type, 
                this->nodes[i].x,
                this->nodes[i].y,
                this->nodes[j].x,
...
double VRP::get_previous_route_length() {
    return this->previous_total_route_length;
}

/*
bool VRP::accept_solve(bool accept = true) {
    if (accept)
        return true;
    else {
        try {
            this->import_solution_buff(this->previous_sol_buff);
            this->total_route_length = this->previous_total_route_length;
            return true;
        }
        catch (const std::exception&) {
            printf("Failed restoring solution");
            return false;
        }
    }
}
*/
bool VRP::reject_move() {
    try {
        this->import_solution_buff(this->previous_sol_buff);
        this->total_route_length = this->previous_total_route_length;
        return true;
...
void VRP::set_max_num_routes(int num) {
    this->max_number_routes = num;
}
...
bool VRP::check_move(VRPMove *M, int rules)
{
    ...
    if (M->total_number_of_routes > this->max_number_routes) {
        if (M->total_number_of_routes > this->get_total_number_of_routes())
            //printf( "rejected route numbers \n");
            return false;
    }
...
int VRP::targeted_osman_perturb(int num, int *nodelist, double alpha)
{
    ///
    /// Perturbs the existing solution by attempting to 
    /// move num nodes from the provided nodelist
    ...
    for (int i = 1; i <= num; i++)
    {
        // Select next node to move from provided list
        k=nodelist[i - 1];
    ...
    // We were able to move tot nodes
    return tot;
